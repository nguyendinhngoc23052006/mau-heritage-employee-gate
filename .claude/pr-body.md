Closes the 24-item gap list from the PR #20 post-mortem in one PR: privilege-escalation and RLS regressions, phantom/misleading RPC failures, data-integrity fixes (NULL variance, dedupe, audit trail), UX gaps (dead locale button, growing-not-shrinking flag queue), and the PR-hygiene misses (stale PR body, empty reviewer verdicts, workflow version drift) Reviewer 3 flagged.
...
**What changed:**
- **Security-hardening:** `memberships_manager_update`'s self-branch now pins `active`/`role` (no self-reactivation) and re-derives the owner-role ceiling so a manager can no longer promote anyone — including themselves — to owner; `store_applications` DELETE is scoped to the applicant's own `declined`/`withdrawn` rows only.
- **Phantom-fix:** `claim_slot` raises a machine-readable error on a lost claim race instead of returning a silent all-null row, and the client now shows a distinguishable message instead of one generic toast for every failure; `close_stale_clock_event` now refuses to double-close an already-paired event and resolves the attendance flag that triggered it instead of stacking a new one on every click.
- **Data-integrity:** `sales_reports.variance_cents` is `NULL` (not `0`) when no expected amount was entered, so the large-variance approval guard can no longer be silently bypassed by an empty field; `apply_manual_rule` is now one atomic, dedupe-keyed RPC instead of a three-step client-side sequence; the forgotten-shift sweep window widened from 48 hours to 30 days; audit-log triggers and notification producers now actually populate the previously-empty audit and notifications tables.
- **UX:** the profile locale-switch button is wired (previously dead); the attendance-flags queue now shrinks as items are resolved instead of only growing; auto rule-detection and analytics cards are honestly badged "not implemented" / "coming soon" rather than presenting stub data as live.
- **Docs+CI:** this PR body replaces the stale PR #20 description; `.claude/review/` is populated with fresh sql-rls-rpc, ts-react, and demo-readiness verdicts; `apply-migrations.yml` now pins `actions/checkout@v7` to match `ci.yml`.

**What you do next:**
1. Review the Cloudflare Pages preview.
2. Merge to main.
3. Open **Supabase Dashboard → SQL Editor** and paste-and-run the entire contents of `supabase/migrations/20260819120000_close_all_gaps.sql`, reproduced verbatim below:
...
4. In `.github/workflows/apply-migrations.yml`, no dashboard action needed — the version bump is code-only and takes effect on the next push to main.
...
- To reverse the schema (only if you must — data is preserved otherwise):
  ```sql
  drop trigger if exists notify_sales_decision on public.sales_reports;
  drop function if exists public.notify_sales_decision();
  drop trigger if exists fanout_announcement_insert on public.announcements;
  drop function if exists public.fanout_announcement();
  drop trigger if exists audit_attendance_flags on public.attendance_flags;
  drop trigger if exists audit_rule_events on public.rule_events;
  drop trigger if exists audit_prize_fine_events on public.prize_fine_events;
  drop trigger if exists audit_sales_reports on public.sales_reports;
  ...
  drop function if exists public.close_stale_clock_event(uuid, timestamptz);
  drop function if exists public.apply_manual_rule(uuid, uuid, text, text);
  drop policy if exists store_applications_self_delete on public.store_applications;
  revoke delete on public.store_applications from authenticated;
  alter table public.sales_reports drop column if exists variance_cents;
  -- auto_clockout_stale and claim_slot and memberships_manager_update were
  -- CREATE OR REPLACE / DROP POLICY + CREATE POLICY over prior versions —
  -- reversing them means restoring the prior migration's function/policy
  -- bodies (20260818130000_lock_writes_and_reviewer_fixes.sql), not a plain drop.
  ```
