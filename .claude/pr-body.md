...
**What you do next:** This preview cannot be exercised: it runs against the production DB, where the migration has not applied yet, so the new `sector_memberships` read fails and the app shows an error card instead of a page — expected for any PR that adds a table on Supabase Free. Review the diff and this description, then merge. The `apply-migrations` workflow applies the migration on merge; confirm it landed by reloading production, signing in, and seeing "Tổ chức" in the header — that link only appears when the DB reports you at tier 1. No env or secret action is needed.
